...
- **OpenRouter free slot fixed.** The chain's OpenRouter model `meta-llama/llama-3.3-70b-instruct:free` was retired from the free tier (404 "unavailable for free"). Tested the free menu on a real briefing excerpt: `google/gemma-4-26b-a4b-it:free` is the only clean non-reasoning prose model (no markdown, numbers written out, ~16 s); GLM-5.2, MiniMax M2.7 and Nemotron 3 Super spend most of the budget on a reasoning channel (Nemotron leaked "the user wants me to…" into content at a small budget). Gemma is now the default; `OPENROUTER_MODEL` overrides. Because free slots go "temporarily rate-limited upstream" for minutes at a time, the OpenRouter rung sends a `models` array (Gemma 4 26B → Gemma 4 31B → Nemotron 3 Super) so OpenRouter routes to the next free model server-side instead of the whole rung failing on one 429 — verified live: Gemma 429'd, Nemotron answered clean in 2.5 s at $0. Key added to the briefing `.env` by Bradley 2026-09-03; chain confirmed seeing it (rungs: groq → cloudflare → openrouter → local). Bradley's OpenRouter key exists in `~/.hermes/.env` but was never in the briefing's `.env` — the chain reads only its own `.env`, so it must be copied there (read per call; no daemon restart needed). Ollama Cloud key in Hermes lists models but 401s on completions — not usable.
