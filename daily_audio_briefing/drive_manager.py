...
def file_exists_in_folder(filename: str, folder_id: str) -> Optional[str]:
    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2 and ('ssl' in str(e).lower() or 'connection' in str(e).lower()):
                _cached_service = None  # Force fresh connection
                import time
...
def upload_file(local_path: str, folder_id: str, skip_existing: bool = True) -> Dict[str, Any]:
    ...
    for attempt in range(3):
        try:
            _cached_service = None  # Force fresh connection on each attempt
            service = get_drive_service()
    ...
    return {
        'name': filename,
        'status': 'error',
        'reason': f'Failed after {attempt + 1} attempts: {last_err}'
    }
