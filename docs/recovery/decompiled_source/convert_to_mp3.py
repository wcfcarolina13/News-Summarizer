...
# File: convert_to_mp3.pyc (Python 3.12)
...
def convert_wav_to_mp3(input_file, output_file, bitrate = (None, '128k')):
...
def convert_folder(folder_path, bitrate, delete_wav = ('128k', False)):
    ...
    for wav_file in wav_files:
        input_size = os.path.getsize(wav_file) / 1048576
        if convert_wav_to_mp3(wav_file, bitrate = bitrate):
            output_file = os.path.splitext(wav_file)[0] + '.mp3'
            output_size = os.path.getsize(output_file) / 1048576
...
def main():
    ...
    parser.add_argument('-b', '--bitrate', default = '128k', help = 'MP3 bitrate (default: 128k). Higher = better quality but larger file')
    parser.add_argument('--delete-wav', action = 'store_true', help = 'Delete original WAV files after conversion')
