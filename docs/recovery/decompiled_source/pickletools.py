...
# File: pickletools.pyc (Python 3.12)
...
def assure_pickle_consistency(verbose = (False,)):
    ...
    for name in pickle.__all__:
        ...
        if isinstance(picklecode, bytes) or len(picklecode) != 1:
            if verbose:
        ...
        if picklecode in copy:
            ...
            d = copy[picklecode]
            if d.name != name:
                raise ValueError(f'''for pickle code {picklecode!r}, pickle.py uses name {name!r} but we\'re using name {d.name!r}''')
            del copy[picklecode]
            continue
        raise ValueError(f'''pickle.py appears to have a pickle opcode with name {name!r} and code {picklecode!r}, but we don\'t''')
    if copy:
        msg = [
...
def dis(pickle, out, memo, indentlevel, annotate = (None, None, 4, 0)):
